...
func WalkFunc(path string, info os.FileInfo, err error) error {
	...
	log.Println(path)
	tensor, err := makeTensorFromImage(path)
	if err != nil {
		log.Println(err)
		return nil
	}
...
func main() {
	...
	modeldir := flag.String("brains", goof.HomeDirectory()+"/brains", "Directory containing the trained model files. The directory will be created and the model downloaded into it if necessary")
	...
	filepath.Walk(imagefile, WalkFunc)
}
